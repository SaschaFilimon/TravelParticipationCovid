...
#### Load Data ##########################################################
reise_20_roh <- read.spss(
  "~/RProjekte/Reiseanalyse 2020/Gesis-2020.sav",
  to.data.frame = TRUE
)
# View(reise_20_roh)
colnames(reise_20_roh)
colnames(reise_20_roh)[1001:ncol(reise_20_roh)]

which(colnames(reise_20_roh) == "F1a")
lapply(reise_20_roh[, 1:which(colnames(reise_20_roh) == "F1a")], unique)


## relevant variables
reise_20 <- reise_20_roh %>% select(
  "gereist_roh" = "F1a",
  "geschlecht" = "S1",
  "alter" = "Alt_num",
  "herkunft" = "P_1",
  "ortsgroesse" = "P_4",
  "haushaltsgroesse" = "HHG_num",
  "kind_alter_0_5" = "S10_1",
  "kind_alter_6_13" = "S10_2",
  "kind_alter_14_17" = "S10_3",
  "bildung_befragter" = "S6a", # for 'bildung'
  "bildung_hauptverdiener" = "S6b", # for 'bildung'
  "berufst_befragter" = "S7a", # for 'vollberufstaetig'
  "berufst_hauptverdiener" = "S7b", # for 'vollberufstaetig'
  "haushaltseinkommen" = "HHEK_num",
  "migration_ja" = "STAATS3_01", # for 'migration'
  "migration_nein" = "STAATS3_18", # for 'migration'
  "migration_ka" = "STAATS3_19", # for 'migration'
  "gereist_01" = "R01b1", # for 'gereist'
  "gereist_02" = "R02b1", # for 'gereist'
  "gereist_03" = "R03b1", # for 'gereist'
  "gereist_04" = "R04b1", # for 'gereist'
  "gereist_05" = "R05b1", # for 'gereist'
  "gereist_06" = "R06b1", # for 'gereist'
  "gereist_07" = "R07b1", # for 'gereist'
  "gereist_08" = "R08b1", # for 'gereist'
  "gereist_09" = "R09b1", # for 'gereist'
  "gereist_10" = "R10b1", # for 'gereist'
  "gereist_11" = "R11b1", # for 'gereist'
  "gereist_12" = "R12b1" # for 'gereist'
)
...
# ('vollberufstaetig')
(levels_berufst <- levels(reise_20$berufst_befragter))
levels(reise_20$berufst_hauptverdiener)

ist_berufst_befragter <- reise_20$berufst_befragter == levels_berufst[1]
ist_berufst_hauptverdiener <- reise_20$berufst_hauptverdiener == levels_berufst[1]

reise_20$vollberufstaetig <- ifelse(
...
# ('bildung')
(levels_bildung <- levels(reise_20$bildung_befragter))
head(reise_20$bildung_befragter)
bildung_befragter_num <- as.numeric(reise_20$bildung_befragter)
head(bildung_befragter_num)

levels(reise_20$bildung_hauptverdiener)
head(reise_20$bildung_hauptverdiener)
bildung_hauptverdiener_num <- as.numeric(reise_20$bildung_hauptverdiener)
...
reise_20$bildung <- sapply(bildung_num, function(i) {
  levels_bildung[i]
}) %>% factor(levels = levels_bildung)

# check
# cbind(bildung_befragter_num, bildung_hauptverdiener_num)[100:105, ]
# reise_20$bildung[100:105]

reise_20 <- reise_20 %>% select(-bildung_befragter, -bildung_hauptverdiener)


## determine: any migration background? ('migration')
head(select(reise_20, migration_ja, migration_nein, migration_ka))
apply(select(reise_20, migration_ja, migration_nein, migration_ka), 2, unique)

migration_ja <- reise_20$migration_ja == "Migrationshintergrund"
migration_nein <- reise_20$migration_nein == "Migrationshintergrund"
migration_ka <- reise_20$migration_ka == "Migrationshintergrund"
...
reise_20$migration <- migration

reise_20 <- reise_20 %>% select(
...
sum(is.na(reise_20))
lapply(reise_20, unique)
# missing values in the variable 'migration' found
sum(is.na(reise_20$migration)) # 176
# exclude observations with missing values
dim(reise_20)
reise_20_mit_migration_na <- reise_20
reise_20 <- reise_20[!is.na(reise_20$migration), ]
dim(reise_20)


## 'gereist_roh' as numeric
head(reise_20$gereist_roh, 20)
reise_20$gereist_roh <- as.numeric(reise_20$gereist_roh == "Ja")
head(reise_20$gereist_roh, 20)


## size of town as numeric
levels(reise_20$ortsgroesse)
head(reise_20$ortsgroesse)
ortsgroesse_fac <- factor(
  reise_20$ortsgroesse,
  labels = c("3000", "17500", "65000", "200000", "1000000")
) %>%
  as.character() %>%
  as.numeric()
head(ortsgroesse_fac)
reise_20$ortsgroesse <- ortsgroesse_fac


## variables concerning children in the household
str(reise_20)
reise_20 <- reise_20 %>% mutate(
...
summary(reise_20$alter)
sum(reise_20$alter < 18)
sum(reise_20$alter > 85)
...
# needed variables
gereist_matrix <- reise_20 %>%
  select(gereist_01:gereist_12)
str(gereist_matrix)

# as numeric values
gereist_matrix_num <- gereist_matrix %>%
  mutate_all(as.numeric)
head(gereist_matrix_num, 15)

# Did the person travel between March and December?
gereist_matrix_bin <- gereist_matrix_num >= 3
...
reise_20 <- reise_20 %>%
  select(-gereist_01:-gereist_12) %>%
  mutate(gereist_roh = as.numeric(gereist_nach_feb)) %>%
  rename(gereist = gereist_roh)
str(reise_20)


## set household sizes greater that 5 to 5
## (for comparability with Reiseanalyse 2021, as the original data in 2021 does)
str(reise_20)
table(reise_20$haushaltsgroesse)
reise_20$haushaltsgroesse[reise_20$haushaltsgroesse > 5] <- 5
table(reise_20$haushaltsgroesse)
...
variablen_verteilung_20 <- variablen %>% lapply(function(x) {
  if (x %in% c("alter", "haushaltseinkommen", "ortsgroesse")) {
    ...
    reise_20 %>% ggplot(aes_string(x)) +
      geom_bar() +
      stat_count(
        geom = "text", colour = "white", size = 4,
...
variablen_verteilung_20_grid <- marrangeGrob(
  variablen_verteilung_20,
  layout_matrix = lay_mat
)
variablen_verteilung_20_grid
# ggsave("central_tendency_and_measure_of_dispersion_20.pdf", variablen_verteilung_20_grid,
...
reise_20$gereist <- as.factor(reise_20$gereist)
levels(reise_20$gereist) <- c("no", "yes")

# split dataset to validation+training and testing set
# (in the end 60% should be used for training, 20% for validating, 20% for testing)
set.seed(123456)
reise_20_split <- initial_split(reise_20, prop = 0.8)
reise_20_train_val <- training(reise_20_split)
reise_20_test <- testing(reise_20_split)

# set trainControl to use 75% of the validation+training set for training
# and the rest for validating
n_train_val <- nrow(reise_20_train_val)
reise_20_index_val <- sample(n_train_val, 0.25 * n_train_val)
train_control <- trainControl(
  method = "cv", number = 1,
  index = list(Fold1 = seq_len(n_train_val)[-reise_20_index_val]),
  returnData = FALSE
)

# define the tuning grid
search_grid <- expand.grid(
  n.trees = seq(50, 1000, by = 100),
...
gbm_tuning_reise_20 <- readRDS("~/RProjekte/Reiseanalyse 2020/gbm_tuning_reise_20_ACC_20210511_final_durchgelaufen.rds")

set.seed(456123)
gbm_tuning_reise_20 <- caret::train(gereist ~ .,
  data = reise_20_train_val,
  method = "gbm",
...
gbm_tuning_reise_20
# saveRDS(gbm_tuning_reise_20, file = "gbm_tuning_reise_20_ACC_20210511_final_durchgelaufen.rds")

print(gbm_tuning_reise_20$finalModel)
par(mar = c(5, 25, 4, 2))
summary(gbm_tuning_reise_20$finalModel, las = 1)
par(mar = c(5, 4, 4, 2))


# train the model with optimized hyperparameters
gbm_tuning_reise_20$bestTune

head(reise_20_train_val$gereist)
reise_20_train_val$gereist <- ifelse(reise_20_train_val$gereist == "no", 0, 1)

set.seed(78912123)
gbm_reise_20 <- gbm(gereist ~ .,
  distribution = "bernoulli",
  data = reise_20_train_val[-reise_20_index_val, ],
  n.trees = gbm_tuning_reise_20$bestTune$n.trees,
  interaction.depth = gbm_tuning_reise_20$bestTune$interaction.depth,
  shrinkage = gbm_tuning_reise_20$bestTune$shrinkage,
  n.minobsinnode = gbm_tuning_reise_20$bestTune$n.minobsinnode
)
# saveRDS(gbm_reise_20, "gbm_reise_20_ACC_20210520_FINAL.rds")
# saved on 05/20/2021 as final model

print(gbm_reise_20)
par(mar = c(5, 10, 4, 2))
summary(gbm_reise_20, las = 1)
par(mar = c(5, 4, 4, 2))


# confusion matrix
gbm_reise_20_response <- predict.gbm(gbm_reise_20,
  newdata = reise_20_test,
  type = "response"
)

gbm_reise_20_schwelle <- mean(gbm_reise_20_response)
gbm_reise_20_schwelle

gbm_reise_20_prognose <- as.factor(ifelse(
  gbm_reise_20_response > gbm_reise_20_schwelle, "yes", "no"
))
confusionMatrix(gbm_reise_20_prognose, reise_20_test$gereist)

# AUC
gbm_reise_20_testing <- prediction(gbm_reise_20_response, reise_20_test$gereist)
gbm_reise_20_roc <- performance(gbm_reise_20_testing, "tpr", "fpr")
plot(gbm_reise_20_roc)
gbm_reise_20_auc_temp <- performance(gbm_reise_20_testing, "auc")
(gbm_reise_20_auc <- as.numeric(gbm_reise_20_auc_temp@y.values))

# another ROC plot
# pdf(file = "roc_reise_20.pdf", width = 10, height = 8)
roc(reise_20_test$gereist, gbm_reise_20_response,
...
# bernoulli response
plot(density(predict(gbm_reise_20,
  n.trees = gbm_tuning_reise_20$bestTune$n.trees,
  type = "response"
)), main = "Bernoulli - Response")

# plot partial dependence plots
plot.gbm(gbm_reise_20, i.var = "geschlecht", type = "response")
plot.gbm(gbm_reise_20, i.var = "alter", type = "response")
plot.gbm(gbm_reise_20, i.var = "herkunft", type = "response")
plot.gbm(gbm_reise_20, i.var = "ortsgroesse", type = "response")
plot.gbm(gbm_reise_20, i.var = "haushaltsgroesse", type = "response")
plot.gbm(gbm_reise_20, i.var = "kind_alter_0_5", type = "response")
plot.gbm(gbm_reise_20, i.var = "kind_alter_6_13", type = "response")
plot.gbm(gbm_reise_20, i.var = "kind_alter_14_17", type = "response")
plot.gbm(gbm_reise_20, i.var = "haushaltseinkommen", type = "response")
plot.gbm(gbm_reise_20, i.var = "vollberufstaetig", type = "response")
plot.gbm(gbm_reise_20, i.var = "bildung", type = "response")
plot.gbm(gbm_reise_20, i.var = "migration", type = "response")
...
## check residuals
plotres(gbm_reise_20, which = 1:9, grid.col = TRUE)


## 2D-plots as above
# using  mean values:
plotmo(gbm_reise_20,
  type = "response", persp.ticktype = "detailed", ylim = c(0.4, 1),
...
plotmo(gbm_reise_20,
  pmethod = "partdep", persp.ticktype = "detailed", ylim = c(0.4, 1),
  degree1 = FALSE, degree2 = c("haushaltsgroesse", "bildung"), all2 = 2, ndiscrete = 10,
...
plotmo(gbm_reise_20,
  pmethod = "partdep", persp.ticktype = "detailed", ylim = c(0.4, 1),
  degree1 = FALSE, degree2 = c("geschlecht", "alter"), all2 = 2
)
...
## reduce to 2D-plots using heatmaps
plotmo(gbm_reise_20,
  pmethod = "partdep", type2 = "image", persp.ticktype = "detailed",
  ylim = c(0.4, 1), degree1 = FALSE, degree2 = "haushaltsgroesse", all2 = 2
)
plotmo(gbm_reise_20,
...
str(reise_20_train_val)

## variables with fewer or equal 5 possible outcomes
training_data <- reise_20_train_val[-reise_20_index_val, -1]
str(training_data)
...
haushaltseinkommen_seq <- seq(min(training_data$haushaltseinkommen),
  max(training_data$haushaltseinkommen),
  length.out = 50
)
...
#### Model GLM ###########################################################
reise_20_train <- reise_20_train_val[-reise_20_index_val, ]

glm_reise_20 <- glm(gereist ~ .,
  data = reise_20_train,
  family = "binomial"
)
summary(glm_reise_20)
Anova(glm_reise_20)

## model quality
glm_reise_20_response <- predict(glm_reise_20, reise_20_test, type = c("response"))
reise_20_test$glm_reise_20_response <- glm_reise_20_response

# pseudo R^2
with(summary(glm_reise_20), 1 - deviance / null.deviance)

# ROC
glm_reise_20_roc <- roc(gereist ~ glm_reise_20_response, data = reise_20_test)
plot(glm_reise_20_roc, print.auc = TRUE)
auc(glm_reise_20_roc)

# confusion matrix
(glm_reise_20_schwelle <- as.numeric(coords(
  glm_reise_20_roc, "best",
  ret = "threshold", transpose = FALSE
)))

glm_reise_20_prognose <- ifelse(
  reise_20_test$glm_reise_20_response > glm_reise_20_schwelle, "yes", "no"
) %>%
  as.factor() %>%
  relevel(ref = "no")

reise_20_test$glm_reise_20_prognose <- glm_reise_20_prognose

confusionMatrix(reise_20_test$gereist,
  reise_20_test$glm_reise_20_prognose,
  positive = "yes"
)
...
#### Model GAM ###########################################################
str(reise_20)
gam_reise_20 <- gam(gereist ~ s(alter, bs = "ps") + geschlecht + herkunft +
  ortsgroesse + haushaltsgroesse + kind_alter_0_5 + kind_alter_6_13 + kind_alter_14_17 +
  s(haushaltseinkommen, bs = "ps") + vollberufstaetig + bildung + migration,
data = reise_20_train,
family = binomial
)
summary(gam_reise_20)
...
## model quality
gam_reise_20_response <- predict(gam_reise_20, reise_20_test, type = c("response"))
reise_20_test$gam_reise_20_response <- gam_reise_20_response

# ROC
gam_reise_20_roc <- roc(gereist ~ gam_reise_20_response, data = reise_20_test)
plot(gam_reise_20_roc, print.auc = TRUE)
auc(gam_reise_20_roc)

# confusion matrix
(gam_reise_20_schwelle <- as.numeric(coords(
  gam_reise_20_roc, "best",
  ret = "threshold", transpose = FALSE
)))

gam_reise_20_prognose <- ifelse(
  reise_20_test$gam_reise_20_response > gam_reise_20_schwelle, "yes", "no"
) %>%
  as.factor() %>%
  relevel(ref = "no")

reise_20_test$gam_reise_20_prognose <- gam_reise_20_prognose

confusionMatrix(reise_20_test$gereist,
  reise_20_test$gam_reise_20_prognose,
  positive = "yes"
)
...
viz_object <- getViz(gam_reise_20)
